...
from vllm.forward_context import BatchDescriptor, get_forward_context, set_forward_context, logger
...
def _to_runtime_moe_comm_type(moe_comm_type: MoECommType | None):
    if moe_comm_type is None:
        return None

    from vllm_ascend.ascend_forward_context import MoECommType as RuntimeMoECommType

    if isinstance(moe_comm_type, RuntimeMoECommType):
        return moe_comm_type

    try:
        return RuntimeMoECommType[moe_comm_type.name]
    except (AttributeError, KeyError) as exc:
        raise ValueError(f"Unsupported MoE communication type: {moe_comm_type}") from exc
...
def set_ascend_forward_context(
    ...
    with set_forward_context(**forward_context_kwargs):
        ...
        forward_context.use_offload_tp = use_offload_tp
        forward_context.additional_kwargs["use_offload_tp"] = use_offload_tp

        from vllm_ascend.ops.fused_moe.moe_comm_method import get_moe_comm_method

        max_num_tokens = int(num_tokens_across_dp.max().item()) if num_tokens_across_dp is not None else num_tokens
        moe_comm_type = _to_runtime_moe_comm_type(
            select_moe_comm_method(max_num_tokens, vllm_config, is_draft_model)
        )

        forward_context.moe_comm_type = moe_comm_type
        forward_context.moe_comm_method = get_moe_comm_method(moe_comm_type)
        logger.info(f"Using MoE communication method: {forward_context.moe_comm_method}, type: {moe_comm_type}")   
