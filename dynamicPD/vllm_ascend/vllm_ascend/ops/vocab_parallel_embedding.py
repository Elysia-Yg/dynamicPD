...
class AscendLogitsProcessorPatch(dynamicPDPatch[AscendLogitsProcessor]):
    def _get_logits(
        ...
        if lmhead_tp_enable():
            return self._get_logits_lmheadtp(hidden_states, lm_head, embedding_bias, use_offload=use_offload)
        else:
            return self._get_logits_normal(hidden_states, lm_head, embedding_bias, use_offload=use_offload)
    ...
    def _get_logits_lmheadtp(
        ...
        gathered_hidden_states = get_lmhead_tp_group(use_offload).all_gather(hidden_states, dim=0)
        local_logits = lm_head.quant_method.apply(lm_head, gathered_hidden_states, bias=embedding_bias)
        # Gather logits for tensor parallel
        logits = get_lmhead_tp_group(use_offload).all_to_all(local_logits)
