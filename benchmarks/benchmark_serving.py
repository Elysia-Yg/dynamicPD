...
# LOG_FILE = f"benchmark_serving_log_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"
LOG_DIR = f"./logs/benchmark_serving/{datetime.now().strftime('%Y-%m-%d')}"
...
async def benchmark(
    ...
    with open(LOG_FILE, 'w') as f:
        f.write("request_id, success, prompt_len, output_len, ttft_ms, tpot_ms, itl_ms_list\n")
        
        for request_id, output in zip (request_ids_list, outputs):
            if output.success:
                tpot = 0
                if output.output_tokens > 1:
                    tpot = (output.latency - output.ttft) / (output.output_tokens - 1)
                itl_str = ":".join([f"{x*1000}" for x in output.itl])
                line = (f"{request_id}, True, {output.prompt_len}, {output.output_tokens}, {output.ttft * 1000}, {tpot * 1000} , {itl_str}\n")
                f.write(line)
            else:
                line = f"{request_id}, False, 0, 0, 0, 0, 0, \"\"\n"
                f.write(line)
...
def main(args: argparse.Namespace):
    ...
    LOG_FILE = LOG_DIR + (
        f"benchmark_serving_log_{args.model}_qps{args.request_rate}_"
