...
class ProxyState:
    ...
    def __init__(self, prefiller_instances, decoder_instances):
        ...
        self.request_timestamps = {}  # five time storage
        self.log_file = LOG_FILE
        with open(self.log_file, "a") as f:
            f.write("request_id,user_to_proxy,proxy_to_prefill,prefill_to_proxy,proxy_to_decode,decode_to_proxy,latency_12,latency_23,latency_34,latency_45\n")
    
    def record_timestamp(self, request_id: str, step_index: int):
        """Record timestamp for a specific step in the request processing."""
        now = time.perf_counter()
        if request_id not in self.request_timestamps:
            self.request_timestamps[request_id] = [0.0] * 5
        self.request_timestamps[request_id][step_index] = now
        if step_index == 4:
            self.flush_log(request_id)
            
    def flush_log(self, request_id: str):
        """Flush the recorded timestamps to the log file."""
        if request_id in self.request_timestamps:
            ts_list = self.request_timestamps[request_id]
            latency_list = [ts_list[i] - ts_list[i - 1] for i in range(1, 5)]
            log_line = f"{request_id}," + ",".join(map(str, ts_list)) + "," + ",".join(map(str, latency_list)) + "\n"
            with open(self.log_file, "a") as f:
                f.write(log_line)
...
async def _handle_select_instance(api: str, req_data: Any,
                                  request_length: int, request_id: str):
    ...
    proxy_state.record_timestamp(request_id, 1)  # proxy_to_prefill
    ...
    proxy_state.record_timestamp(request_id, 2)  # prefill_to_proxy
    proxy_state.release_prefiller(prefiller_idx, prefiller_score)
    response_json = response.json()
    # print(f"Prefiller response: {response_json}, {response_json['choices'][0]['finish_reason']}")
...
async def _handle_completions(api: str, request: Request):
    ...
    try:
        ...
        proxy_state.record_timestamp(request_id, 0)  # user_to_proxy
        ...
        instance_info = InstanceInfo(request_id=request_id,
                            prefiller_idx=prefiller_idx,
                            prefiller_score=prefiller_score,
                            prefiller=prefiller,
                            decoder=decoder,
        ...
        async def generate_stream():
            ...
            try:
                ...
                prefill_response = await instance_info.prefiller.client.post(
                    api,
                    json=req_data_copy,
                    headers=headers)
                ...
                # print("kv transfer params:", kv_transfer_params)
                ...
                proxy_state.record_timestamp(request_id, 3)  # proxy_to_decode
                while retry:
                    retry = False
                    async for chunk in stream_service_response_with_retry(
                        ...
                        yield chunk
                proxy_state.record_timestamp(request_id, 4)  # decode_to_proxy
            except Exception as e:
                logger.error(
...
if __name__ == '__main__':
    ...
    import uvicorn
    uvicorn.run(app, host=global_args.host, port=global_args.port,  limit_max_requests=10000)